...
public abstract class MLVBLiveRoom {
     ...
     * @return
     */
    public abstract Size getPlaySize();

    /**
     * 自定义视频采集，向 SDK 发送自己采集的 YUV 视频数据。
     * @param buffer 	视频数据。
     * @param bufferType 视频格式 目前仅支持 YUV_420P 、RGB_RGBA 两种数据格式。
     ...
     */
    public abstract int customerDataPush(byte [] buffer, int bufferType, int w, int h);

    /**
     * 自定义视频采集，向 SDK 发送自己采集的 texture 视频数据
     * @param textureID 视频纹理 ID
     * @param w 视频图像的宽度，不能大于 TXLivePushConfig 中的 videoResolution 属性设定的宽度，否则会失败，小于时 SDK 会自动裁剪。
     * @param h 视频图像的高度，不能大于 TXLivePushConfig 中的 videoResolution 属性设定的宽度，否则会失败，小于时 SDK 会自动裁剪。
     * @return
     */
    public abstract int customerTexturePush(int textureID, int w, int h);

    /**
     * fixme 重载该方法 使用自己的LivePlayView
     * 进入房间（观众调用）
     *
     * 观众观看直播的正常调用流程是：
     * 1.【观众】调用 getRoomList() 刷新最新的直播房间列表，并通过 {@link IMLVBLiveRoomListener.GetRoomListCallback} 回调拿到房间列表。
